...
class PLANNER(object):
    ...
    def Start_GUI(self):
        ...
        # ________________________________________ BUTTONS''DEFINITIONS

        # Plot Robot button
        self.RobotButton = tk.Button(self.topLevelWindow, width=20, text="Plot Previous Robot's Path", bg='green',
                                     activebackground='green', command=self.togglePlotRobot)
        self.countRobot = 0

        # Plot Sensor button
        self.ExecuteButton = tk.Button(self.topLevelWindow, width=20, text='Execute Robot Command ', bg='blue',
                                       activebackground='blue', command=self.startPlotExecute)
        self.countExecute = 0
        # Plot Map button
        self.MapButton = tk.Button(self.topLevelWindow, width=20, text='Plot Map', bg='green', activebackground='green',
                                   command=self.togglePlotMap)
        self.countMap = 0
        # Plot Path button
        self.PathButton = tk.Button(self.topLevelWindow, width=20, text="Plot Previous Robot's Path", bg='blue',
                                    activebackground='blue', command=self.togglePlotPath)
        self.countPath = 0
        ...
        self.Movement = tk.Checkbutton(self.topLevelWindow, text="Show robot's single movements", variable=self.var_mov,
                                       command=command_mov)
        self.Movement.deselect()
        self.var_mov.set(0)
        ...
        self.sensor = tk.Checkbutton(self.topLevelWindow, text="show sensors", variable=self.var_sensor,
                                     command=command_sensor)
        self.sensor.select()
        self.var_sensor.set(1)
        ...
        self.noise = tk.Checkbutton(self.topLevelWindow, text="Add noise", variable=self.add_noise,
                                    command=command_add_noise)
        self.noise.select()
        self.add_noise.set(1)
        ...
        # _________________________________________G R I D S ___
    ...
    def sendInputs_client1(self):
        ...
        try:
            ...
            resp = sendInputs(origin_x, origin_y, origin_angRob, dest_x, dest_y, sensorBool, num_sensorsInt,
                              angle_sensor_orig,
                              range_angleRob, radiusRob, advance, max_angle, num_steps, select, largest_sensor,
                              pathNAme, fileNAme, flg_noise)
